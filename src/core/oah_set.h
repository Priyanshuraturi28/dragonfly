...
class OAHSet {  // Open Addressing Hash Set
 ...
 public:
  ...
  // Result of a single SIMD probe over a run of OAHEntry lanes.
  struct LaneMasks {
    // Lanes that are non-empty AND whose stored ext-hash matches the probed
    // hash or is lazily zero (uninitialized cache). These are key-comparison
    // candidates: the probe narrows the search, the caller confirms by key.
    uint32_t candidates;
    // Lanes that are empty (data_ == 0). Used by Add to pick an insertion slot.
    uint32_t empties;
  };
  ...
  // (VectorWide) scans. Defined and explicitly instantiated in oah_set.cc.
  ...
  // out-of-line entry point over the FORCE_INLINE AddImpl. Defined in oah_set.cc.
  ...
  // SIMD strides. Defined in oah_set.cc.
  ...
  // Defined in oah_set.cc, where its Add() calls fold in via FORCE_INLINE.
 ...
 private:
  ...
  // The body of Add. Defined `inline FORCE_INLINE` in oah_set.cc so it folds
  // into both the out-of-line Add and AddMany's bulk-insert loop; the inline
  // keyword keeps it non-interposable, which is what allows always_inline to
  // apply to an out-of-line definition.
  bool AddImpl(std::string_view str, uint32_t ttl_sec);

  // A located entry: `slot` is the matched OAHEntry (possibly empty if it just
  // expired), living at entries_[bucket_id] (vector position `pos`); null if the
  // key is absent.
  struct MatchResult {
    OAHEntry* slot;
    uint32_t bucket_id;
  ...
  // Probes the displacement window at `bid` (candidate bits from a prior
  // ProbeLanes) then the extension vector at `ext_bid` for `str`, applying expiry
  // on the match. The shared core of AddImpl and FindInternal. Defined in
  // oah_set.cc.
  ...
  // Find and Erase; defined in oah_set.cc.
  ...
  // A probe candidate whose key did not match: refresh its stale/lazy-zero
  // ext-hash cache (set after a rehash) so future probes skip it, then apply
  // pending expiry.
  FORCE_INLINE void RefreshStaleCandidate(OAHEntry& e, uint64_t ext_hash) {
