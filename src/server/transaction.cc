    ...
    IterateActiveShards([&](auto& sd, ShardId i) {
      if (disarmed_shards.test(i)) {
        ...
        shard_set->Add(i, [this, context] {
          EngineShard::tlocal()->PollExecution(context, nullptr);
          intrusive_ptr_release(this);
        });
      }
    });
...
bool Transaction::ScheduleInShard(EngineShard* shard, bool execute_optimistic) {
  ...
  if (!IsGlobal()) {
    ...
    // Check if we can run immediately. Skip if another transaction is already running
    // on this shard (e.g. an inlined tx preempted on the connection fiber).
    if (lock_granted && execute_optimistic && shard->running_tx() == nullptr) {
